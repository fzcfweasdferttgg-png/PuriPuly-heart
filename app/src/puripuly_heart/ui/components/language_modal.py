...
class LanguageModal:
    ...
    def open(
        ...
        Args:
            ...
            none_option: If set, show this label as the first item (selects code "").
        """
        # Build recent grid (up to 6 items)
        recent_grid = self._build_recent_grid(recent, current)

        # Build language list (1-column grid style)
        lang_list = self._build_language_list(current, none_option=none_option)
    ...
    def _build_language_list(self, current: str, *, none_option: str | None = None) -> ft.ListView:
        """Build scrollable list of all languages (Bento Card Style)."""
        items = []

        # Optional "none" item at the top (e.g. "Disabled")
