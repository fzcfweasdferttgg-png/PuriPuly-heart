...
class LocalGigaamRnntInferenceError(RuntimeError):
    """Raised when GigaAM RNNT inference fails for an utterance."""
...
class LocalGigaamRnntSTTBackend(STTBackend):
    model_dir: Path
    ...
    def __post_init__(self) -> None:
        if self.sample_rate_hz != GIGAAM_RECOGNIZER_SAMPLE_RATE_HZ:
            raise ValueError(f"sample_rate_hz must be {GIGAAM_RECOGNIZER_SAMPLE_RATE_HZ}")
    ...
    async def open_session(self) -> STTBackendSession:
        ...
        return _LocalGigaamRnntSession(backend=self)
    ...
    def _create_recognizer(self) -> object:
        try:
            return create_local_gigaam_rnnt_recognizer(
            ...
            raise LocalGigaamRnntLoadError(str(exc)) from exc
    ...
    async def decode_f32(self, samples_f32: np.ndarray) -> str:
        ...
        async with self._decode_lock:
            try:
                ...
                raise LocalGigaamRnntInferenceError(str(exc)) from exc
    ...
    def _decode_f32_sync(self, recognizer: object, samples_f32: np.ndarray) -> str:
        ...
        stream.accept_waveform(GIGAAM_RECOGNIZER_SAMPLE_RATE_HZ, samples)
...
@dataclass(slots=True)
class _LocalGigaamRnntSession(STTBackendSession):
    backend: LocalGigaamRnntSTTBackend
