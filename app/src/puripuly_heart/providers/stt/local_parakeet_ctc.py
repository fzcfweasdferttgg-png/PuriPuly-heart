...
class LocalParakeetCtcInferenceError(RuntimeError):
    """Raised when Parakeet CTC inference fails for an utterance."""
...
def _audio_diag_prefix(stream_label: str | None) -> str:
    prefix = "[AudioDiag][local_parakeet_ctc]"
    if stream_label:
        return f"{prefix}[{stream_label}]"
    return prefix
...
class LocalParakeetCtcSTTBackend(STTBackend):
    model_dir: Path
    ...
    def __post_init__(self) -> None:
        if self.sample_rate_hz != PARAKEET_CTC_SAMPLE_RATE_HZ:
            raise ValueError(f"sample_rate_hz must be {PARAKEET_CTC_SAMPLE_RATE_HZ}")
    ...
    async def open_session(self) -> STTBackendSession:
        ...
        return _LocalParakeetCtcSession(backend=self)
    ...
    def _create_recognizer(self) -> object:
        try:
            return create_local_parakeet_ctc_recognizer(
            ...
            raise LocalParakeetCtcLoadError(str(exc)) from exc
    ...
    async def decode_f32(self, samples_f32: np.ndarray) -> str:
        ...
        async with self._decode_lock:
            try:
                ...
                raise LocalParakeetCtcInferenceError(str(exc)) from exc
    ...
    def _decode_f32_sync(self, recognizer: object, samples_f32: np.ndarray) -> str:
        ...
        stream.accept_waveform(PARAKEET_CTC_SAMPLE_RATE_HZ, samples)
...
@dataclass(slots=True)
class _LocalParakeetCtcSession(STTBackendSession):
    backend: LocalParakeetCtcSTTBackend
