...
class LocalParakeetTdtInferenceError(RuntimeError):
    """Raised when Parakeet TDT inference fails for an utterance."""
...
def _audio_diag_prefix(stream_label: str | None) -> str:
    prefix = "[AudioDiag][local_parakeet_tdt]"
    if stream_label:
        return f"{prefix}[{stream_label}]"
    return prefix
...
class LocalParakeetTdtSTTBackend(STTBackend):
    model_dir: Path
    ...
    def __post_init__(self) -> None:
        if self.sample_rate_hz != PARAKEET_TDT_SAMPLE_RATE_HZ:
            raise ValueError(f"sample_rate_hz must be {PARAKEET_TDT_SAMPLE_RATE_HZ}")
    ...
    async def open_session(self) -> STTBackendSession:
        ...
        return _LocalParakeetTdtSession(backend=self)
    ...
    def _create_recognizer(self) -> object:
        try:
            return create_local_parakeet_tdt_recognizer(
            ...
            raise LocalParakeetTdtLoadError(str(exc)) from exc
    ...
    async def decode_f32(self, samples_f32: np.ndarray) -> str:
        ...
        async with self._decode_lock:
            try:
                ...
                raise LocalParakeetTdtInferenceError(str(exc)) from exc
    ...
    def _decode_f32_sync(self, recognizer: object, samples_f32: np.ndarray) -> str:
        ...
        stream.accept_waveform(PARAKEET_TDT_SAMPLE_RATE_HZ, samples)
...
@dataclass(slots=True)
class _LocalParakeetTdtSession(STTBackendSession):
    backend: LocalParakeetTdtSTTBackend
    ...
    def _log_decode_start_diagnostics(self, samples_f32: np.ndarray) -> None:
        with contextlib.suppress(Exception):
            ...
            logger.info(
                "%s decode_start audio_ms=%.1f rms_db=%.1f peak_db=%.1f zero_ratio=%.3f",
    ...
    def _log_decode_done_diagnostics(
        ...
        *,
        ...
        with contextlib.suppress(Exception):
            logger.info(
                "%s decode_done audio_ms=%.1f inference_ms=%.1f rtf=%.3f text_len=%s empty_result=%s suspicious_repetition=%s",
                _audio_diag_prefix(self.backend.stream_label),
                audio_ms,
                inference_ms,
                rtf,
