...
class PromptAssemblyCache:
    ...
    template: str
    dual_template: str = ""
    target_language_rules: Mapping[str, str] = MappingProxyType({})
    language_pair_examples: Mapping[str, str] = MappingProxyType({})
...
def _load_prompt_cache() -> PromptAssemblyCache:
    ...
    dual_template_path = prompts_dir / "translation_prompt_dual.md"
    dual_template = _read_prompt_text(dual_template_path) if dual_template_path.exists() else ""
...
def build_translation_prompt_variables(
    ...
    *,
    ...
    if second_target_name:
        variables["secondTargetName"] = second_target_name
    return variables
...
def render_dual_translation_prompt_template(
    *,
    source_name: str,
    target_name: str,
    second_target_name: str,
) -> str:
    """Render the dual-language translation prompt template with dynamic variables."""
    cache = _get_prompt_cache()
    template = cache.dual_template
    if not template:
        raise FileNotFoundError("Dual translation prompt template not found")
    rendered = template
    for key, value in build_translation_prompt_variables(
        source_name, target_name, second_target_name=second_target_name
    ).items():
        rendered = rendered.replace(f"${{{key}}}", value)
    return rendered
