...
_REQUIRED_RUNTIME_DLLS_ALWAYS = (
    "onnxruntime.dll",
    "onnxruntime_providers_shared.dll",
)
_REQUIRED_RUNTIME_DLLS_GPU = ("DirectML.dll",)


def _is_cpu_mode() -> bool:
    return os.environ.get("PURIPULY_MODE", "gpu").lower() == "cpu"


def _required_dlls() -> tuple[str, ...]:
    if _is_cpu_mode():
        return _REQUIRED_RUNTIME_DLLS_ALWAYS
    return _REQUIRED_RUNTIME_DLLS_ALWAYS + _REQUIRED_RUNTIME_DLLS_GPU
