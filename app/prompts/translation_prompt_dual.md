...
Interpret the current ${sourceName} text to translate into ${targetName} and ${secondTargetName} naturally, preserving the speaker's social attitude and emotion.
...
* Translate `<input>` into both ${targetName} and ${secondTargetName}.
* Return only the two translations on a single line, separated by a space. No labels, no explanations, no line breaks.
